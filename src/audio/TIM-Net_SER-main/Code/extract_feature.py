...
parser.add_argument('--data_name', type=str, default="SIMS")
args = parser.parse_args()
def read_txt_file(file_path: str) -> Tuple[list[str], list[str]]:
        file_paths = []
        labels = []
...
def generate_csv(csv_save:str, data_name: str="EMODB", feature_type: str="MFCC", embed_len: int = 39, mean_signal_length:int = 96000, class_labels: Tuple = ("angry", "boredom", "disgust", "fear", "happy", "neutral","sad")):
    ...
    name='dataset/audio/train_list.txt'
    
    i=1
    file_paths, labels = read_txt_file('dataset/audio/test_list.txt')
    temp_size_ = True
    for filename,label in tqdm(zip(file_paths,labels)):
        directory=class_labels[label]
        feature_vector = get_feature(file_path = filename, feature_type=feature_type, mean_signal_length=mean_signal_length, embed_len = embed_len)
        if temp_size_:
            print(f"### Feature Size:{feature_vector.shape} ###")
            temp_size_ = False
        if not os.path.exists(csv_save + "/" + directory):
            os.makedirs(csv_save + "/" + directory)
        np.savetxt(csv_save + "/" + directory + "/audio" + str(i) + '_raw' + '.csv', feature_vector, delimiter = ',')
        i+=1
...
SIMS_LABEL = ("negative", "natural","positive")
LABEL_DICT = {"CASIA":CASIA_LABEL,"EMODB":EMODB_LABEL,"IEMOCAP":IEMOCAP_LABEL,"EMOVO":EMOVO_LABEL,"SAVEE":SAVEE_LABEL,"RAVDE":RAVDE_LABEL,"SIMS":SIMS_LABEL}
PATH_DICT = {"CASIA":"./CASIA_MFCC_88","EMODB":"./EMODB_MFCC_96","IEMOCAP":"./IEMOCAP_MFCC_310","EMOVO":"./EMOVO_MFCC_96","SAVEE":"./SAVEE_MFCC_130","RAVDE":"./RAVDE_MFCC_110","SIMS":"./SIMS_MFCC_96"}
